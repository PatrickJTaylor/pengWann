...
def build_geometry(
    seedname: str, path: str = ".", cell: ArrayLike | None = None
) -> Structure:
    """
    ...
    cell : array_like or None, optional
        The cell vectors associated with the structure. Defaults to None, in which case
        the cell vectors will automatically be extracted from a seedname.win file in
        the same directory as the xyz file.
    ...
    Notes
    -----
    If you intend to use the output of this function as an input to the
    ...
    translated_home_cell = true, then the Wannier centre assignments made by this
    function will still be correct, but the interactions identified by the
    :py:func:`~pengwann.geometry.identify_interatomic_interactions` function may not
    be accurate (more specifically, you may end up calculating interations between the
    wrong periodic images).
    """
    symbols, coords = read_xyz(f"{path}/{seedname}_centres.xyz")
